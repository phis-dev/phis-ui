...
- `/sitemap.xml` and `/robots.txt` are route handlers from `@phis/ui/next/seo-routes`, answered from the Public bridge only.
  - The sitemap exists when the Site has a public base and the Public Area has both `index` and `sitemap` switched on; otherwise it answers 404. Its candidates are the Area root, the exact Module routes and the published Public pages, which the Public Area read adds on `include=publishedPages`; each candidate is resolved as an anonymous page view and listed only when that view is not a redirect, not refused and not `noindex`. Every page appears once per locale with the full hreflang set.
  - The finished sitemap is kept until the next publish. Every request reads the Public Area with its published pages once and fingerprints the answer -- live revision ids, Area preset, active Modules, locales, public base; only a changed fingerprint resolves the candidates again. The Core never notifies a Site, so the cache is pulled, not pushed, and a new Site process starts empty.
...
- A listener reading signals addressed to an address declares that address (third argument to `usePhiSignalListener`, or `receiver` in its filter). Delivery is gated on the listeners registered per address, so a listener that only compares `signal.receiver` in its body has told the bus nothing and its signals are held forever without a warning. Listening in on an address another listener owns is fine -- registration is per address, not per listener. `pnpm runtime-modules:check` enforces this.
