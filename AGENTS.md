...
- Use the existing runtime signal/bus system for cross-widget, inspector, and shell coordination.
  ...
  - The bus mints a fresh correlation id whenever none is passed, so an answer that omits the field is not uncorrelated -- it looks correlated and is not, and a listener filtering on it silently never matches. A signal sent while handling another therefore passes `signal.correlationId` explicitly, including through a helper it calls and across an `await`. Beginning a new exchange is written out as `createPhiSignalCorrelationId()`; a correlation id is never a string of one's own. `scripts/validate-signal-correlation-contracts.ts` enforces both.
  - A listener that reads signals addressed to an address must name that address -- as the third argument to `usePhiSignalListener` or as `receiver` in its filter. The bus holds an addressed signal until a listener answers for its receiver and counts only declared addresses, so comparing `signal.receiver` in the body alone leaves every signal sent there waiting, silently and forever: nothing throws, nothing is dropped, and the sender's own state moves on so its half keeps looking right. Reading an address somebody else owns is sound -- delivery is decided once per address, not per listener. `scripts/validate-signal-address-contracts.ts` enforces it and records the deliberate listen-ins.
  - A `condition/reload` is answered with `usePhiRuntimeConditionStateResponder`, never by hand. It replies to the asker with the asked correlation id, and takes a getter for state a Controller holds in a ref.
  ...
  - one module owns at most one controller type, while every Widget, Layout, controller, form/provider definition, data/option provider, and Calendar adapter has exactly one `ownerModuleId`; `controllerType`, `controller`, `controllerMountPolicy`, `controllerDefinition`, and the `Controller` Client form an atomic optional contract; a separately activatable controller or lifecycle requires a separate module, and reuse across domains belongs in Core or a separate module, never in multiple ownership
...
- In `/builder/phis/ui/pages`, workspace chrome above the page canvas is not the selected page's `header_bottom`; the selected page `header_bottom` remains a normal page-owned CMS region with its own root tree.
...
- Area-shell, route, and theme presets are distinct descriptor families. Every descriptor has one stable `(ownerModuleId, presetKey)` identity and a positive integer version; one route preset belongs to exactly one Area and one effective normalized path. The Builder addresses a Module Page by `createPhiPresetCmsPageId(ownerModuleId, presetKey)` -- a hash of that same pair, so it is recomputable rather than stored and never moves when a path is reassigned. A Site Page is addressed by its path, which is the identity the server keeps for it.
- Outside Public, a route answers under its owner's package: `@acme/status/modules/auth` declaring `/providers` is served at `/acme/status/providers`. Public carries no namespace, and `/` is an application for the Area root slot rather than a route of the Module's own. Area route mounts are explicit exported injection points that place a navigation entry inside a container; they compose navigation, never paths.
