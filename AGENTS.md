...
- Ant Design is imported only inside the canonical `Phi*Control` adapters and root/theme adapters. When a
  ...
  `Tour`. The validator names 41 controlled primitives, closes five more to a single owner file
  (`soleOwnerPrimitives`), and permits everything it does not name -- so seven others, `Space`,
  `Layout`, `List`, `Collapse` and the rest, may still be imported directly today. That exception is on its way out: see
